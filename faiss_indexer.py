    ...
    messages=[
        {
            ...
            "content": (
                ...
                # 不確定な情報に対する慎重な対応
                "原則として提供した資料に基づいて回答してください．回答に必要な関連情報が不十分な場合には、アルゴリズムとデータ構造に関する一般的な知識を用いて補完しても構いません。"
                "推測に基づく不確定な情報の提供は行わないでください。"
        ),
    },
        {
            "role": "user",
            "content": f"以下の資料に基づいて回答してください。回答に必要な関連情報が不十分な場合には、アルゴリズムとデータ構造に関する一般的な知識を用いて補完しても構いません:\n\n{content}\n\n質問: {query}"
        }
    ]
)
